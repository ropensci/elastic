...
#' 
#' ## Search many indices
#' Search_uri(x, index = "gbif")$hits$total
#' Search_uri(x, index = "shakespeare")$hits$total
#' Search_uri(x, index = c("gbif", "shakespeare"))$hits$total
#' 
#' ## search_type
...
#'
#' ## sorting
#' Search_uri(x, index="shakespeare", type="act", sort="text_entry")
#' Search_uri(x, index="shakespeare", type="act", sort="speaker:desc", fields='speaker')
#' Search_uri(x, index="shakespeare", type="act",
#'  sort=c("speaker:desc","play_name:asc"), fields=c('speaker','play_name'))
#'
#' ## paging
#' Search_uri(x, index="shakespeare", size=1, fields='text_entry')$hits$hits
#' Search_uri(x, index="shakespeare", size=1, from=1, fields='text_entry')$hits$hits
#'
#' ## queries
#' ### Search in all fields
#' Search_uri(x, index="shakespeare", type="act", q="york")
#'
#' ### Searchin specific fields
#' Search_uri(x, index="shakespeare", type="act", q="speaker:KING HENRY IV")$hits$total
#'
#' ### Exact phrase search by wrapping in quotes
#' Search_uri(x, index="shakespeare", type="act", q='speaker:"KING HENRY IV"')$hits$total
#'
#' ### can specify operators between multiple words parenthetically
#' Search_uri(x, index="shakespeare", type="act", q="speaker:(HENRY OR ARCHBISHOP)")$hits$total
#'
#' ### where the field line_number has no value (or is missing)
#' Search_uri(x, index="shakespeare", q="_missing_:line_number")$hits$total
#'
#' ### where the field line_number has any non-null value
#' Search_uri(x, index="shakespeare", q="_exists_:line_number")$hits$total
#'
#' ### wildcards, either * or ?
#' Search_uri(x, index="shakespeare", q="*ay")$hits$total
#' Search_uri(x, index="shakespeare", q="m?y")$hits$total
#'
#' ### regular expressions, wrapped in forward slashes
#' Search_uri(x, index="shakespeare", q="text_entry:/[a-z]/")$hits$total
#'
#' ### fuzziness
#' Search_uri(x, index="shakespeare", q="text_entry:ma~")$hits$total
#' Search_uri(x, index="shakespeare", q="text_entry:the~2")$hits$total
#' Search_uri(x, index="shakespeare", q="text_entry:the~1")$hits$total
#'
#' ### Proximity searches
#' Search_uri(x, index="shakespeare", q='text_entry:"as hath"~5')$hits$total
#' Search_uri(x, index="shakespeare", q='text_entry:"as hath"~10')$hits$total
#'
#' ### Ranges, here where line_id value is between 10 and 20
#' Search_uri(x, index="shakespeare", q="line_id:[10 TO 20]")$hits$total
#'
#' ### Grouping
#' Search_uri(x, index="shakespeare", q="(hath OR as) AND the")$hits$total
#'
