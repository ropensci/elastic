...
#' invisible(docs_bulk_delete(df, index = 'foobar', type = 'foobar'))
#' Search("foobar", asdf = TRUE)$hits$total
#' 
#' # delete by passing in doc ids
#' docs_bulk_delete(df, index = 'foobar', type = 'foobar', doc_ids = df$id)
#' Search("foobar", asdf = TRUE)$hits$total
#' }
docs_bulk_delete <- function(x, index = NULL, type = NULL, chunk_size = 1000,
                             doc_ids = NULL, raw = FALSE, ...) {
  
  UseMethod("docs_bulk_delete")
}

#' @export
docs_bulk_delete.default <- function(x, index = NULL, type = NULL, 
