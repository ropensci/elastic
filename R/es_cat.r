...
#'
#' @examples \dontrun{
#' es_cat() # list Elasticsearch cat endpoints
#' es_cat('aliases')
...
#' es_cat('plugins')
#' es_cat('recovery', verbose=TRUE)
#' es_cat('recovery', index='mran')
#' es_cat('thread_pool')
#' es_cat('thread_pool', verbose=TRUE)
#' es_cat('shards')
#' es_cat('fielddata')
#' es_cat('fielddata', fields='body')
#' }

es_cat <- function(what='', verbose=FALSE, index=NULL, fields=NULL)
{
  conn <- es_get_auth()
  ...
  args <- es_compact(list(v = if(verbose) '' else NULL, fields=fields))
