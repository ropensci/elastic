...
#' docs_bulk_index(df, 'foobar', 'foobar')
#' docs_bulk_index(df, 'foobar', 'foobar', es_ids = FALSE)
#' Search("foobar", asdf = TRUE)$hits$hits
#' 
#' # more examples
#' docs_bulk_index(mtcars, index = "hello", type = "world")
#' ## field names cannot contain dots
#' names(iris) <- gsub("\\.", "_", names(iris))
#' docs_bulk_index(iris, "iris", "flowers")
#' ## type can be missing, but index can not
#' docs_bulk_index(iris, "flowers")
#' ## big data.frame, 53K rows, load ggplot2 package first
#' # res <- docs_bulk_index(diamonds, "diam")
#' # Search("diam")$hits$total
#' }
docs_bulk_index <- function(x, index = NULL, type = NULL, chunk_size = 1000,
  doc_ids = NULL, es_ids = TRUE, raw = FALSE, quiet = FALSE, ...) {
  
  UseMethod("docs_bulk_index")
}

#' @export
docs_bulk_index.default <- function(x, index = NULL, type = NULL, 
